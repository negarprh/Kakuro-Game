...
- `User`:
- Implemented as `User` dataclass (`kakuro/models/domain.py`) and persisted in `users` table (`kakuro/db/schema.sql`) through `kakuro/models/user.py`.

- `RegisteredUser`:
- Dataclass exists in `kakuro/models/domain.py`, but login flow does not instantiate it.
- Runtime representation is session-based: `session["user_id"]` set and `session["is_guest"]=False`.

- `GuestUser`:
- Dataclass exists in `kakuro/models/domain.py`, but guest flow uses session flags.
- Runtime representation is `session["is_guest"]=True` with username `"Guest"`.

- `GameSession`:
- Fully implemented dataclass (`kakuro/models/domain.py`) with `status`, `board`, `elapsedTime`, `isPaused`, `isSubmitted`, `result`.
- Persisted in Flask server session as serialized dict (`kakuro/services/game_service.py`).

- `Board`:
- Implemented dataclass with `cells`, `size`, helper methods (`matrix`, `get_cell`, validation helpers).
- Generated by `kakuro/services/board_generator.py`.

- `Cell`:
- Base dataclass in `kakuro/models/domain.py`.

- `PlayCell`:
- Implemented subclass with editable value fields and optional `hinted` flag.
- Used for playable cells from board generation.

- `ClueCell`:
- Implemented subclass for clue/blocked cells with `clueDown` and `clueRight`.

- `SavedGame`:
- Implemented as dataclass in `kakuro/models/domain.py` plus persistence functions in `kakuro/models/saved_game.py`.
- Persisted in SQLite `saved_games` table with serialized `board_state`.

- `Result`:
- Implemented dataclass in `kakuro/models/domain.py`.
- Created on solved submit path in `game_service.submit_solution(...)`.

- `DifficultyLevel enum`:
- Implemented as `DifficultyLevel` enum in `kakuro/models/domain.py`.
- `GameSession.__post_init__` normalizes difficulty to enum-backed string values.
